...
from user.views import CustomLoginView  # Importa a view 'CustomLoginView' do módulo 'views' da aplicação 'user'. Esta é a view que você criou para a página de login.
...
urlpatterns = [
    ...
    # Inclui todas as URLs da aplicação 'user'. Isso significa que todas as rotas definidas em 'user/urls.py' serão incluídas no projeto principal.
      ...
      path('', CustomLoginView.as_view(), name='login'),  # Define a página de login como a URL raiz
